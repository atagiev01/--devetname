...
import { X, Code, Palette, Layout, Sliders, Code2, Music, Play, Pause } from 'lucide-react';
import { WEDDING_MUSIC_PRESETS } from '../data/weddingMusicPresets';
import {
  HEADING_FONTS,
  BODY_FONTS,
  buildHeadingStack,
...
>>>>>>> d6f4fdc (Initial commit)
  ...
  const [activeTab, setActiveTab] = useState<'general' | 'design' | 'music' | 'css'>('general');
  const [headingFont, setHeadingFont] = useState(
    template.themeConfig?.fontsCustomized ? firstFamily(template.themeConfig?.fontFamilyHeading) : ''
  );
  const [bodyFont, setBodyFont] = useState(
    template.themeConfig?.fontsCustomized ? firstFamily(template.themeConfig?.fontFamilyBody) : ''
  );
  const [musicEnabled, setMusicEnabled] = useState<boolean>(!!template.music);
  const [music, setMusic] = useState(template.music || '');
  const [musicTitle, setMusicTitle] = useState(template.musicTitle || '');
  ...
  const togglePreview = () => {
    ...
    if (!audio || !music.trim()) return;
    if (isPlaying) {
      audio.pause();
      setIsPlaying(false);
    } else {
      audio.src = music.trim();
      audio
        .play()
...
>>>>>>> d6f4fdc (Initial commit)
        ...
        themeConfig: {
          ...
          accentColor,
          customCss,
          fontsCustomized: !!(headingFont || bodyFont),
          ...(headingFont ? { fontFamilyHeading: buildHeadingStack(headingFont) } : {}),
          ...(bodyFont ? { fontFamilyBody: buildBodyStack(bodyFont) } : {}),
        },
      };
...
>>>>>>> d6f4fdc (Initial commit)
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  <label className="block text-xs text-stone-400 mb-1">Köhnə Qiymət (endirim üçün)</label>
                  <input
                    type="number"
                    min="0"
                    step="0.01"
                ...
                <div>
                  <label className="block text-xs text-stone-400 mb-1">Valyuta</label>
                  <select
                    value={currency}
          ...
          {activeTab === 'design' && (
            <div className="space-y-4 animate-fade-in">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-stone-400 mb-1">Başlıq Şrifti</label>
                  <select
                    value={headingFont}
                    onChange={(e) => setHeadingFont(e.target.value)}
                    className="w-full px-4 py-2.5 rounded-xl bg-stone-950 border border-stone-800 text-xs focus:border-amber-400 focus:outline-none"
                  >
                    <option value="">Standart (şablonun öz şrifti)</option>
                    {HEADING_FONTS.map((f) => (
                  ...
                  {headingFont && (
                    <p className="mt-2 text-lg text-stone-200" style={{ fontFamily: buildHeadingStack(headingFont) }}>
                      Əli & Nigar — Şən Çiçək Ğ Ö Ü İ ı
                    </p>
                  )}
                </div>
                <div>
                  <label className="block text-xs text-stone-400 mb-1">Mətn Şrifti</label>
                  <select
                    value={bodyFont}
                  ...
                  {bodyFont && (
                    <p className="mt-2 text-sm text-stone-200" style={{ fontFamily: buildBodyStack(bodyFont) }}>
                      Əli & Nigar — Şən Çiçək Ğ Ö Ü İ ı
                    </p>
                  )}
                </div>
          ...
          {activeTab === 'music' && (
            <div className="space-y-4 animate-fade-in">
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-stone-950 border border-stone-800">
                <div>
                  <p className="text-xs font-semibold text-stone-200">Bu şablon üçün standart musiqi</p>
                  <p className="text-[11px] text-stone-500">
                    Bu şablonla yeni dəvətnamə yaradanda musiqi avtomatik seçilir (dəvətnamədə dəyişmək olar).
                  </p>
                </div>
                <input
                  type="checkbox"
              ...
              {musicEnabled && (
                <div className="space-y-3">
                  <div>
                    <label className="block text-xs text-stone-400 mb-1">Hazır musiqidən seç</label>
                    <select
                      value=""
                      onChange={(e) => {
                        const track = WEDDING_MUSIC_PRESETS.find((t) => t.id === e.target.value);
                        if (track) {
                          setMusic(track.url);
                          setMusicTitle(track.defaultTitle);
                        }
                      }}
                      className="w-full px-4 py-2.5 rounded-xl bg-stone-950 border border-stone-800 text-xs focus:border-amber-400 focus:outline-none"
                    >
                      <option value="">— Siyahıdan seçin —</option>
                      {WEDDING_MUSIC_PRESETS.map((t) => (
                  ...
                  <div>
                    <label className="block text-xs text-stone-400 mb-1">
                      Musiqi faylı / linki (.mp3) — məsələn <span className="font-mono">/music/qizil.mp3</span> və ya tam https linki
                    </label>
                    <div className="flex gap-2">
                      <input
                        ...
                        placeholder="/music/bu-sablon.mp3"
                        className="flex-1 px-4 py-2.5 rounded-xl bg-stone-950 border border-stone-800 text-xs focus:border-amber-400 focus:outline-none"
                      />
                      <button
                        type="button"
                        onClick={togglePreview}
                        className="px-3 rounded-xl bg-stone-800 hover:bg-stone-700 text-amber-300 text-xs font-medium flex items-center gap-1.5"
                      >
                        {isPlaying ? <Pause className="w-3.5 h-3.5" /> : <Play className="w-3.5 h-3.5" />}
                        <span>{isPlaying ? 'Dayandır' : 'Dinlə'}</span>
                      </button>
                    </div>
                  </div>

                  <div>
                    <label className="block text-xs text-stone-400 mb-1">Musiqinin adı</label>
                    <input
                      type="text"
                      value={musicTitle}
                      onChange={(e) => setMusicTitle(e.target.value)}
                      placeholder="Məs: Romantik Piano"
                      className="w-full px-4 py-2.5 rounded-xl bg-stone-950 border border-stone-800 text-xs focus:border-amber-400 focus:outline-none"
                    />
                  </div>

                  <p className="text-[11px] text-stone-500">
                    Hər şablon üçün fərqli musiqi qoymaq üçün mp3 faylını layihədəki <span className="font-mono">public/music/</span>{' '}
                    qovluğuna atın və yuxarıya <span className="font-mono">/music/fayl-adi.mp3</span> yazın.
                  </p>
                </div>
              )}
            </div>
