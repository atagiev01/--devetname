...
const formatPrice = (tpl: Template) => {
  ...
  const currency = tpl.currency || 'AZN';
            ...
            >
              ...
              <div className="relative aspect-[16/10] bg-stone-950 overflow-hidden group">
                ...
                <div className="absolute top-3 right-3">
                  <span
                    className={`text-[10px] font-bold px-2.5 py-1 rounded-full backdrop-blur-md flex items-center gap-1 ${
                      formatPrice(tpl)
                        ? 'bg-stone-950/80 text-amber-300 border border-amber-500/30'
                        : 'bg-stone-950/80 text-stone-400 border border-stone-700'
                    }`}
                  >
                    <Tag className="w-3 h-3" />
                    <span>{formatPrice(tpl) || 'Qiymət yoxdur'}</span>
                  </span>
                </div>
                ...
                {tpl.oldPrice ? (
                  <div className="flex items-center gap-2 text-[11px]">
                    <span className="text-stone-500 line-through">
                      {tpl.oldPrice} {tpl.currency === 'USD' ? '$' : tpl.currency === 'EUR' ? '€' : '₼'}
                    </span>
